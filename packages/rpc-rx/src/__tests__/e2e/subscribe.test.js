...
describe.skip('e2e subscriptions', () => {
  ...
  it('retrieves current timestamp', done => {
    ...
    api.state.subscribeStorage([[storage.timestamp.now]]).subscribe(data => {
  ...
  it('retrieves multiple values', done => {
    api.state
      .subscribeStorage([[storage.timestamp.now], [storage.session.sessionLength], [storage.staking.sessionsPerEra]])
      .subscribe(data => {
