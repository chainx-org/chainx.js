...
describe('Bytes', () => {
  ...
  describe('construction', () => {
    it('decodes when hex is not length prefixed', () => {
      expect(new Bytes('0x3a636f6465').toU8a()).toEqual(CODE);
    });
    it('creates via storagedata (no prefix)', () => {
      expect(new Bytes(new StorageData('0x3a636f6465')).toU8a()).toEqual(CODE);
    });
    it('creates via storagedata (with length prefix)', () => {
      expect(new Bytes(new StorageData('0x143a636f6465')).toU8a()).toEqual(CODE);
    });
  });
});
