...
describe('EventRecord', () => {
  beforeEach(() => {
    Event.injectMetadata(new Metadata(metadataRpc));
  });
  it('decodes correctly', () => {
    const records = createType('Vec<EventRecord>', json1.params.result.changes[0][1]);
    const er = records[0];
    expect(er.phase.type).toEqual('ApplyExtrinsic');
  });
  it.skip('decodes more complex events', () => {
    const records = createType('Vec<EventRecord>', json2.params.result.changes[0][1]);

    expect(records).toHaveLength(4);
    const er = records[2];
    expect(er.event.data.toArray().map(v => v.toString())).toEqual([
