...
describe('KeyValue', () => {
  it('decodes KeyValue from u8a', () => {
    expect(
      new KeyValue(
        Uint8Array.from([4 << 2, 0x11, 0x22, 0x33, 0x44, 9 << 2, 0x99, 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11])
      ).toJSON()
    ).toEqual({
      key: '0x11223344',
  ...
  it('encodes KeyValue from JSON', () => {
    expect(
      ...
      new Uint8Array([4 << 2, 0x11, 0x22, 0x33, 0x44, 9 << 2, 0x99, 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11])
    );
  });
...
describe('KeyValueOption', () => {
  it('exposes the properties for key/value', () => {
    const kv = new KeyValueOption(['0x11223344']);
    expect(kv.key.toHex()).toEqual('0x11223344');
