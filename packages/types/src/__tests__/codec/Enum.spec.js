...
describe('Enum', () => {
  ...
  it('provides a clean toString()', () => {
    expect(new Enum(['foo', 'bar']).toString()).toEqual('foo');
  });

  it('provides a clean toString() (enum)', () => {
    expect(new Enum(['foo', 'bar'], new Enum([], 1)).toNumber()).toEqual(1);
  });

  it('converts to and from U8a', () => {
    expect(new Enum(['foo', 'bar'], new Uint8Array([1])).toU8a()).toEqual(new Uint8Array([1]));
  });

  it('converts from JSON', () => {
    expect(new Enum(['foo', 'bar'], 5).toString()).toEqual('5');
  });
});
