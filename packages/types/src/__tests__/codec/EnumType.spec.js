...
describe('Struct', () => {
  it('provides a clean toString() (value)', () => {
    expect(new EnumType({ Text, U32 }, new Uint8Array([0, 2 << 2, 49, 50])).value.toString()).toEqual('12');
  });
  it('provides a clean toString() (enum)', () => {
    expect(new EnumType({ Text, U32 }, new Uint8Array([1, 2 << 2, 49, 50])).toString()).toEqual('U32');
  });
  it('decodes from a JSON input (lowercase)', () => {
    expect(new EnumType({ Text, U32 }, { text: 'some text value' }).value.toString()).toEqual('some text value');
  });
  it('decodes from a JSON input (mixed case)', () => {
    expect(new EnumType({ Text, U32 }, { U32: 42 }).value.toString()).toEqual('42');
  });
  it('decodes from JSON string', () => {
    expect(new EnumType({ Null, U32 }, 'null').type).toEqual('Null');
  });
  ...
  it.skip('allows checking against defined indexes', () => {
    expect(new EnumType({ 1: Text, 5: U32 }, new Uint8Array([1, 2 << 2, 49, 50])).toString()).toEqual('Text');
  });
  it('allows accessing the type and value', () => {
    const text = new Text('foo');
    const enumType = new EnumType({ Text, U32 }, { Text: text });
