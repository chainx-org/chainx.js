...
describe('Vector', () => {
  ...
  it('allows contruction via JSON', () => {
    expect(new Vector(Text, ['6', '7']).toJSON()).toEqual(['6', '7']);
  });
  it('exposes the type', () => {
    expect(vector.Type).toEqual('Text');
  });
  it.skip('decodes a complex type via construction', () => {
    const test = createType(
      'Vec<(PropIndex, Proposal, AccountId)>',
      new Uint8Array([
        4,
        10,
        0,
  ...
  describe('vector-like functions', () => {
    ...
    it('exposes a working reduce', () => {
      expect(vector.reduce((r, e) => `${r}${e}`, '')).toEqual('123345456756789');
    });
  });
  describe('encode', () => {
    ...
    testEncode(
      'toU8a',
      Uint8Array.from([20, 4, 49, 8, 50, 51, 12, 51, 52, 53, 16, 52, 53, 54, 55, 20, 53, 54, 55, 56, 57])
    );
  });
});
