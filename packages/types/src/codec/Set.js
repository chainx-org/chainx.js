...
import { assert, isU8a, isNumber, isUndefined, stringCamelCase, stringUpperFirst, u8aToHex } from '@chainx/util';
...
export default class CodecSet extends Set {
  ...
  static decodeSetArray(setValues, value) {
    return value.reduce((result, key) => {
      assert(
        !isUndefined(setValues[key]),
        `Set: Invalid key '${key}' passed to Set, allowed ${Object.keys(setValues).join(', ')}`
      );
      result.push(key);
      return result;
    }, []);
